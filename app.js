...
function strip(hours) {
  return `<div class="strip">${hours.map(h => {
    const t = new Date(h.time_iso).toLocaleTimeString([], { hour: "numeric" });
    return `<div class="h" title="${h.status}">${t}
      <span class="w">${num(h.wbgt_f)}</span>
      <span>${num(h.aqi_forecast)}</span>
      <span>${num(h.precip_pct, "%")}</span></div>`;
  }).join("")}</div>`;
}
...
function renderScheduled() {
  ...
  body.innerHTML = `
    ...
    <h3>Next 12 hours (WBGT / AQI / precip)</h3>
    ${strip(v.hours || [])}`;
}
